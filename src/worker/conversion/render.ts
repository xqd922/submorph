...
function unsupported(node: ProxyNode, target: OutputTarget): string | undefined {
	...
	if (target === "v2rayng" && ["hysteria2", "socks5", "anytls", "snell"].includes(node.protocol)) return `${node.protocol} is not a verified v2rayNG output`;
	if ("transport" in node && node.transport.type === "xhttp" && node.protocol !== "vless") return "XHTTP only supports VLESS";
	if ("transport" in node && node.transport.type === "xhttp" && target === "singbox") return "sing-box 1.13 cannot express XHTTP";
...
function mihomo(node: ProxyNode): Record<string, unknown> {
	if (node.protocol === "hysteria2") return mihomoHysteria2(node);
	if (node.protocol === "socks5") return mihomoSocks5(node);
	if (node.protocol === "anytls") return renderAnyTlsMihomo(node);
	if (node.protocol === "snell") return renderSnellMihomo(node);
...
function singbox(node: ProxyNode): Record<string, unknown> {
	if (node.protocol === "hysteria2") return singboxHysteria2(node);
	if (node.protocol === "socks5") return singboxSocks5(node);
	if (node.protocol === "anytls") return renderAnyTlsSingBox(node);
	if (node.protocol === "snell") return renderSnellSingBox(node);
...
function uri(node: CoreNode): string {
