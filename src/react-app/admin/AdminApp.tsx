...
const views: Array<{ id: View; label: string; mark: string }> = [
	{ id: "overview", label: "概览", mark: "O" }, { id: "conversions", label: "转换记录", mark: "C" },
	{ id: "links", label: "短链接", mark: "L" }, { id: "blocked", label: "拦截源", mark: "B" }, { id: "audit", label: "审计日志", mark: "A" },
];
...
function Overview() {
	...
	const cards = [["今日转换", num(data, "todayConversions", "conversionsToday", "totalToday"), "次"], ["成功率", num(data, "successRate"), "%"], ["缓存命中率", num(data, "cacheHitRate"), "%"], ["平均延迟", num(data, "averageDurationMs", "avgDurationMs", "averageLatency"), "毫秒"]] as const;
	...
	return <><div className="metric-grid">{cards.map(([label, value, suffix]) => <article key={label}><span>{label}</span><strong>{value.toLocaleString()}<small>{suffix}</small></strong><i /></article>)}</div><div className="overview-grid"><section className="admin-card"><header><div><span>转换活动</span><h2>最近 7 天</h2></div><em>实时</em></header>{trend.length ? <div className="mini-chart">{trend.map((item, index) => { const value = num(item, "count", "total", "value"); return <div key={`${text(item, "date", "label")}-${index}`}><span title={`${value} 次转换`} style={{ height: `${Math.max(8, value / max * 100)}%` }} /><small>{text(item, "label", "day", "date").slice(0, 5)}</small></div>; })}</div> : <EmptyState label="活动" />}</section><section className="admin-card"><header><div><span>运行状态</span><h2>近期错误</h2></div></header>{recent.length ? <div className="error-list">{recent.slice(0, 6).map((item, index) => <div key={`${text(item, "code")}-${index}`}><b>{text(item, "code", "errorCode")}</b><span>{num(item, "count").toLocaleString()}</span></div>)}</div> : <EmptyState label="错误" />}</section></div></>;
}
